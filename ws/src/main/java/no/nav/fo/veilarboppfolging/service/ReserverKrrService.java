...
@Slf4j
public class ReserverKrrService {

    private DigitalKontaktinformasjonService digitalKontaktinformasjonService;

    public ReserverKrrService(DigitalKontaktinformasjonService digitalKontaktinformasjonService) {
    ...
    public HentReservertKrrResponse hentReservertKrr(String fnr) {
        return Try.of(() -> digitalKontaktinformasjonService.erBrukerReservertIKrr(fnr))
                .map(ReserverKrrService::mapToKrrResponse)
                .mapFailure(
                        Case($(instanceOf(NotAuthorizedException.class)), ReserverKrrService::ikkeTilgang),
                        Case($(instanceOf(NotFoundException.class)), ReserverKrrService::ikkeFunnet),
                        Case($(), ReserverKrrService::generellFeil)
                )
                .get();
    }
