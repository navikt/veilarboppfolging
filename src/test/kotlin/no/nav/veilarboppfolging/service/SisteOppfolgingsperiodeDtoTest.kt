...
class SisteOppfolgingsperiodeDtoTest {
    ...
    fun `GjeldendeOppfolgingsperiode with OPPFOLGING_STARTET serializes correctly`() {
        ...
        assertEquals("OPPFOLGING_STARTET", jsonNode["sisteEndringsType"].asString())
        assertEquals(testOppfolgingsperiodeUuid.toString(), jsonNode["oppfolgingsperiodeUuid"].asString())
        assertEquals(testAktorId, jsonNode["aktorId"].asString())
        assertEquals(testIdent, jsonNode["ident"].asString())
        assertEquals(testStartTidspunkt, ZonedDateTime.parse(jsonNode["startTidspunkt"].asString()))
        assertTrue(jsonNode["sluttTidspunkt"].isNull, "sluttTidspunkt should be null for active period")
        assertEquals("NAV Oslo", jsonNode["kontor"]["kontorNavn"].asString())
        assertEquals("0219", jsonNode["kontor"]["kontorId"].asString())
    ...
    fun `GjeldendeOppfolgingsperiode with ARBEIDSOPPFOLGINGSKONTOR_ENDRET serializes correctly`() {
        ...
        assertEquals("ARBEIDSOPPFOLGINGSKONTOR_ENDRET", jsonNode["sisteEndringsType"].asString())
        assertEquals(testOppfolgingsperiodeUuid.toString(), jsonNode["oppfolgingsperiodeUuid"].asString())
        assertEquals(testAktorId, jsonNode["aktorId"].asString())
        assertEquals(testIdent, jsonNode["ident"].asString())
        assertEquals(testStartTidspunkt, ZonedDateTime.parse(jsonNode["startTidspunkt"].asString()))
        assertTrue(jsonNode["sluttTidspunkt"].isNull, "sluttTidspunkt should be null for active period")
        assertEquals("NAV Bergen", jsonNode["kontor"]["kontorNavn"].asString())
        assertEquals("1201", jsonNode["kontor"]["kontorId"].asString())
        assertTrue(jsonNode.has("producerTimestamp"), "producerTimestamp should be present")
    }
    ...
    fun `AvsluttetOppfolgingsperiode serializes correctly`() {
        ...
        assertEquals("OPPFOLGING_AVSLUTTET", jsonNode["sisteEndringsType"].asString())
        assertEquals(testOppfolgingsperiodeUuid.toString(), jsonNode["oppfolgingsperiodeUuid"].asString())
        assertEquals(testAktorId, jsonNode["aktorId"].asString())
        assertEquals(testIdent, jsonNode["ident"].asString())
        assertEquals(testStartTidspunkt, ZonedDateTime.parse(jsonNode["startTidspunkt"].asString()))
        assertEquals(testSluttTidspunkt, ZonedDateTime.parse(jsonNode["sluttTidspunkt"].asString()))
